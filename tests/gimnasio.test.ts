...
describe('Pruebas Integrales del Gimnasio (Cobertura 100%)', () => { 
    ...
    describe('Entidades Socio', () => {
        it('Debe calcular correctamente la cuota VIP y Comun', () => {
            const vip = new SocioVip(1, "Juan", 7000, "Pileta");
            const comun = new SocioComun(2, "Pedro", 5000);
        ...
        it('Debe lanzar error si los datos del socio son invalidos', () => {
            expect(() => new SocioComun(-1, "Error", 5000)).toThrow("El DNI debe ser un numero entero positivo."); 
            expect(() => new SocioComun(1.5, "Error", 5000)).toThrow("El DNI debe ser un numero entero positivo."); 
            expect(() => new SocioComun(1, "", 5000)).toThrow("El nombre no puede estar vacio."); 
            expect(() => new SocioComun(1, "123", 5000)).toThrow("El nombre no puede contener numeros."); 
        ...
        it('Debe incluir informacion de equipamiento en mostrarInfo', () => {
            ...
            const vip = new SocioVip(2, "Maria", 7000, "Spa"); 
    ...
    describe('Control de Acceso y Horarios', () => {
        ...
        it('Debe denegar acceso a un socio que no existe', () => {
            expect(() => gym.validarIngreso(999, lunes10am)).toThrow("[MOLINETE] Acceso Denegado");
        });

        it('Debe eliminar al socio automaticamente si la cuota no esta al dia durante el ingreso', () => {
            const deudor = new SocioComun(1, "Deudor", 5000);
            deudor.cuota = 2000;
            gym.agregarSocio(deudor);
            expect(() => gym.validarIngreso(1, lunes10am)).toThrow("[MOLINETE] Baja Automatica");
            expect(gym.buscarSocio(1)).toBeUndefined();
        });

        it('Debe denegar acceso los domingos', () => {
            gym.agregarSocio(new SocioVip(1, "Juan", 7000));
        ...
        it('Debe permitir acceso directo si todo esta OK', () => {
            gym.agregarSocio(new SocioComun(1, "Juan", 5000));
            expect(gym.validarIngreso(1, lunes10am)).toContain("[MOLINETE] Acceso OK");
            expect(gym.validarIngreso(1, sabado10am)).toContain("[MOLINETE] Acceso OK");
        });
        ...
        it('Debe controlar la permanencia de 1 hora al salir', () => {
            ...
            expect(gym.registrarSalida(1, salidaOk)).toContain("[SISTEMA] Salida OK");
            gym.validarIngreso(1, entrada);
            expect(gym.registrarSalida(1, salidaTarde)).toContain("ADVERTENCIA: Exceso de tiempo");
        });
    });

    describe('Gestion Administrativa', () => {
        it('Debe agregar socios automaticamente segun la cuota', () => {
            const s1 = gym.agregarSocioPorCuota(1, "Comun", 5000);
            const s2 = gym.agregarSocioPorCuota(2, "VIP", 8000, "Spa");
        ...
        it('Debe eliminar socios por tipo independientemente', () => {
            gym.agregarSocio(new SocioComun(1, "Socio Uno", 5000));
            gym.agregarSocio(new SocioVip(2, "Socio Vip Uno", 7000));
        ...
        it('Debe procesar cambios de estado por pago', () => {
            gym.agregarSocio(new SocioComun(1, "Luis", 5000));
